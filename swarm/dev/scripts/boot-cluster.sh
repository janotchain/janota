...
#
# The cluster contains a bootnode, a janetad node and multiple swarm nodes, with
...
# static janetad configuration
JANETAD_IP="192.168.33.3"
JANETAD_RPC_PORT="8545"
JANETAD_RPC_URL="http://${JANETAD_IP}:${JANETAD_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the janetad and
# swarm nodes
start_bootnode() {
...
# start_janetad_node starts a gcm node with --datadir pointing at <base-dir>/janetad
# and a single, unlocked account with password "janetad"
start_janetad_node() {
  local dir="${base_dir}/janetad"
  mkdir -p "${dir}"

  local password="janetad"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating janetad account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get janetad account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${JANETAD_IP}"
    --rpcport   "${JANETAD_RPC_PORT}"
    --verbosity "6"
  )

  start_node "janetad" "${JANETAD_IP}" "$(which janetad)" ${args[@]}
}
...
create_account() {
  ...
  janetad --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
